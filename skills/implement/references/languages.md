...
- 各リポジトリの既存規約、設計、依存、formatter、linter、test 方針を最優先する。
...
- loading、error、empty、権限差分、accessibility、responsive 崩れを実装時の回帰観点として確認する。
...
- unit / integration test、formatter、static analysis、build の対象を plan に記載する。
...
- typecheck、lint、unit test の対象を plan に記載する。
...
- explain、sqlfluff、既存 test、影響件数確認などの検証要否を plan に記載する。
...
- gofmt / go vet / go test / race test の対象を plan に記載する。
...
- cargo fmt / clippy / test の対象を plan に記載する。
